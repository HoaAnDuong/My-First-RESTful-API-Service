...
def get_orders():
    json = get_json_via_query(['OrderID', 'Orders.CustomerID', 'CustomerName', 'EmployeeID', 'EmployeeName', 'ShipperID', 'ShipperName', 'Date', 'PaymentMethod', 'Status', 'Total'],"select Orders.OrderID, Orders.CustomerID, CustomerName, Orders.EmployeeID, printf('%s %s',LastName,FirstName), Orders.ShipperID, ShipperName, Date, PaymentMethod, OrderPayments.Status, Total from Orders inner join Customers on Orders.CustomerID = Customers.CustomerID inner join Employees on Orders.EmployeeID = Employees.EmployeeID inner join Shippers on Orders.ShipperID = Shippers.ShipperID inner join OrderDates on Orders.OrderID = OrderDates.OrderID inner join OrderPayments on Orders.OrderID = OrderPayments.OrderID","Orders")
